...
# ===== USER-DEFINED SEQUENCE FUNCTION =====
def run_user_defined_sequence():
    """Executes the specific flight plan provided by the user."""
    print("\n" + "="*50)
    print("🚀 STARTING USER-DEFINED FLIGHT SEQUENCE")
    print("="*50)
    ...
    try:
        ...
        print("STEP 2: Moving FORWARD for 2.5 seconds...")
        ...
        print("STEP 3: ASCENDING for 2 seconds...")
        ...
        print("STEP 4: Moving BACKWARD for 2.5 seconds...")
        for _ in range(25): # 2.5 seconds
            send_packet(PACKET_BACKWARD)
            time.sleep(0.1)

        # Step 5: Descend for 1.5 sec
        print("STEP 5: DESCENDING for 1.5 seconds...")
        for _ in range(15): # 1.5 seconds
            send_packet(PACKET_DESCEND)
            time.sleep(0.1)

        # Step 6: Forward for 2.5 sec
        print("STEP 6: Moving FORWARD for 2.5 seconds...")
        for _ in range(25): # 2.5 seconds
            send_packet(PACKET_FORWARD)
            time.sleep(0.1)
        
        # Step 7: Final Stabilization & Land
        print("STEP 7: Stabilizing before landing...")
        for _ in range(20): send_packet(PACKET_STOP); time.sleep(0.1) # Hover for 2s
        
        print("STEP 8: Sequence complete. Landing...")
        for _ in range(10): send_packet(PACKET_LAND); time.sleep(0.1)
        time.sleep(3.0)

    finally:
        print("\n--- ✅ USER-DEFINED SEQUENCE COMPLETE ---")
        sequence_running.clear()
        command_queue.put(PACKET_STOP)
    ...
    if connect():
        ...
        try:
            while is_running.is_set():
                ...
                print(f"{'v. Run User-Defined Sequence':<30}")
                print("-" * 60)
