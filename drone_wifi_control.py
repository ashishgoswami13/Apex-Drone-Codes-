        ...
        try:
            ...
            while len(buffer) >= 4 and buffer.startswith(b'CTP:'):
                ...
                if msg.get("op") == "NOTIFY" and "param" in msg:
                    ...
                    # FINAL FIX: Check if the required keys exist before trying to parse them.
                    # This handles both the detailed sensor packets and simple status packets.
                    if "D8" in params and "D9" in params and "D10" in params:
                        # Altitude is a 16-bit signed int, sent as low byte (D8) and high byte (D9)
...
def safe_land_and_exit():
    """
    Stops all movement, lands the drone ONLY IF it's flying, 
    and signals threads to close.
    """
    print("🛑 Initiating safe shutdown...")
    command_queue.put(PACKET_STOP)
    time.sleep(0.5) # Allow time for the stop command to be sent

    # CRITICAL FIX: Only send the land command if the drone is actually flying.
    ...
    is_running.clear() # Signal threads to stop
...
if __name__ == "__main__":
    ...
    if connect():
        ...
        command_queue.put(PACKET_STOP)
        time.sleep(2) # Wait for calibration to complete
